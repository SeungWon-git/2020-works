...
GLfloat light_x = -0.5, light_y = 1.0, light_z = 0.0;

// 이미지
unsigned int texture_a, texture_b, texture_c, texture_d, texture_e, texture_f, texture_line, texture_backstage;
...
void InitTexture() {
	...
	//BackStage
	glGenTextures(1, &texture_backstage); //--- 텍스처 생성
	glBindTexture(GL_TEXTURE_2D, texture_backstage); //--- 텍스처 바인딩
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT); //--- 현재 바인딩된 텍스처의 파라미터 설정하기
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	stbi_set_flip_vertically_on_load(true); //--- 이미지가 거꾸로 읽힌다면 추가
	data = stbi_load("sky.png", &widthImage, &heightImage, &numberOfChannel, 0);
	glTexImage2D(GL_TEXTURE_2D, 0, 4, widthImage, heightImage, 0, GL_RGBA, GL_UNSIGNED_BYTE, data); //---텍스처 이미지 정의 
	//(3번째 인자값 - 이미지 파일의 비트 수준이 24이면 '3'으로 하고 7번째 인자에 'GL_RGB'를 준다.)
	//(3번째 인자값 - 이미지 파일의 비트 수준이 32이면 '4'으로 하고 7번째 인자에 'GL_RGBA'를 준다.)
...
GLvoid drawScene()
{
	...
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, texture_backstage);
