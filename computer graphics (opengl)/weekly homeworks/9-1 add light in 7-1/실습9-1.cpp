...
//법선
GLfloat sphere_n[spherepoints][3] = {};	//--- 노말값 (Normal)
...
int light_cnum = 0;
GLfloat light_x = 0.0f, light_y = 0.0f, light_z = 1.0f;
...
void InitBuffer()
{
	InitVao(solar_vao, solar_vbo, spherepoints, sphere_s, sphere_n);

	InitVao(earth1_vao, earth1_vbo, spherepoints, sphere_s, sphere_n);
	//InitVao(earth2_vao, earth2_vbo, spherepoints, sphere_s, earth_c);
	//InitVao(earth3_vao, earth3_vbo, spherepoints, sphere_s, earth_c);

	InitVao(moon1_vao, moon1_vbo, spherepoints, sphere_s, sphere_n);
...
GLvoid drawScene()
{
	...
	objColorLocation = glGetUniformLocation(s_program, "objectColor");	//--- object Color값 전달
	glUniform3f(objColorLocation, 0, 1, 0);
	...
	mTransform = glm::scale(mTransform, glm::vec3(0.3f, 0.3f, 0.3f));
	modelLoc = glGetUniformLocation(s_program, "model");
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, &mTransform[0][0]);
...
GLvoid Keyboard(unsigned char key, int x, int y)
{
	...
	case 'r':
		light_rad -= 1;
		light_x = glm::cos(glm::radians(light_rad)); light_z = glm::sin(glm::radians(light_rad));
		break;
	case 'R':
		light_rad += 1;
		light_x = glm::cos(glm::radians(light_rad)); light_z = glm::sin(glm::radians(light_rad));
